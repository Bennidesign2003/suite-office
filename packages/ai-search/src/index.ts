/**
 * Search utilities (server side) — Serper Google API first, then Tavily, with
 * DuckDuckGo as the keyless last resort. Runs server side (Node fetch) to
 * avoid browser CORS; the Serper key reuses SERPER_API_KEY, the Tavily key
 * reuses TAVILY_API_KEY.
 *
 * Search is the one part of Suite that can leave the machine. The keyless
 * DuckDuckGo path needs no account, and both keyed backends are opt-in.
 */
...
export { searchProxyUrl, setSearchProxyUrl } from './shared'
