...
describe('generate_deck self-driven page-by-page generation', () => {
  it('plans 5 pages → tool loop calls the LLM 5 times to write pages → lands 5 pages → progress complete', async () => {
    ...
    // The tool internally calls generatePageLocal once per page (the AI never touches per-page work)
    expect(localPageCalls.sort((a, b) => a - b)).toEqual([1, 2, 3, 4, 5])
    // All 5 pages landed
...
describe('generate_deck page generation', () => {
  it('the spec builder generates every page through the marker landing', async () => {
  ...
  it('no page pipeline available → fails fast', async () => {
    const { access } = makeAccess()
...
describe('generate_deck lands pages while generating + retries', () => {
  ...
  it('a permanently failing page is attempted inline twice and once more in the retry round', async () => {
    const { access, getPages, localPageCalls } = makeAccess({ failPages: [2] })
    const skill = createSlidesSkill(access)
    await skill.executeTool(deckCall(3))
    expect(getPages()).toBe(2)
    // Two inline attempts, then the retry round tries the same two again:
    // generation is local and cheap, so a failed page is worth the second pass
    // before the deck gives up on it.
    expect(localPageCalls.filter((n) => n === 2).length).toBe(4)
  })
