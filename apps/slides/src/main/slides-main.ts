...
export function registerSlidesIpc(): void {
  ...
  // ── Single-page generation: a JSON slide spec written by the renderer's LLM
  // call is built directly into a one-slide pptx with pptx-engine primitives —
  // no HTML intermediate. This is the only page pipeline; the hosted one it
  // used to share the marker contract with is gone with Genspark.
  ipcMain.handle(
