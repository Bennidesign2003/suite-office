...
import type { OllamaCatalog } from '@genoffice/ai-provider'
 ...
 * to be signed in to, only a daemon that is either running with models or not.
 */
function DaemonEntry({
  onStatusChange,
}: {
  onStatusChange?: (status: OllamaCatalog | null) => void
}) {
  const { t } = useI18n()
  const [status, setStatus] = useState<OllamaCatalog | null>(null)
  ...
  // re-probe on focus: `ollama serve` and `ollama pull` happen in a terminal
  // next to the app, and the entry should reflect that without a restart
  useEffect(() => {
    let alive = true
    const refresh = () => {
      void window.aiOffice.ollamaStatus?.().then((s) => {
  ...
  const ready = up && modelCount > 0
  const subtitle = !up
    ? t('setAiDaemonDown')
    : modelCount === 0
      ? t('setAiNoModels')
      : t('setAiDaemonUp', { version: status?.version ?? '', n: String(modelCount) })
  ...
  return (
    <div className="account-entry">
      ...
      >
        <span className={`account-avatar${ready ? ' logged-in' : ''}`}>
        ...
        <span className="account-text">
          <span className="account-name">{t('setAiDaemon')}</span>
          {!ready && <span className="account-sub error">{subtitle}</span>}
        </span>
        <svg
